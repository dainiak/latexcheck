...
export default function ResultDisplay({ results, previewHtml, onGotoLine }) {
                    ...
                    MathJax.typesetPromise([containerRef.current]).then(() => {
                        if (previewHtml && containerRef.current) {
                            containerRef.current.innerHTML = containerRef.current.innerHTML
                                .replace(/\\par/g, '<br>')
                                .replace('((beginTask))', `<span class="badge bg-secondary">${i18n.strings.task}</span>`)
                                .replace('((beginSolution))', `<span class="badge bg-secondary">${i18n.strings.solution}</span>`);
                        }
                    });
                }
            });
    ...
    if (previewHtml) {
        return (
            <div className="card-text" ref={containerRef}
    ...
    if (results.length === 0) {
        return <div className="card-text" ref={containerRef}
                    dangerouslySetInnerHTML={{ __html: i18n.strings.noErrors }} />;
    }
                    ...
                    {r.fragments.map((f, fi) => (
                        ...
                        <span
                            ...
                            dangerouslySetInnerHTML={{
                                __html: i18n.strings.wrongFragment.replace('{1}', f.snippet)
